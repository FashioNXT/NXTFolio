////////////////////////////////////////////
// USE LOCAL DATABASE (REQUIRES COUNTRY, STATE, CITY SEEDING)
////////////////////////////////////////////

// $(function () {
// 	// Populate states dropdown when country is selected
...
// 				$('#state').empty();
// 				$('#state').append($('<option>').text("Select State").attr('value', ""));
// 				$('#city').empty();
...
// 	// Populate cities dropdown when state is selected
...
// 					$('#city').append($('<option>').text(city.name).attr('value', city.id));
// 				});
// 			}
// 		});
// 	});
...
// Add event listener to countries dropdown
let country_element = document.getElementById('country');
country_element.addEventListener('change', function() {
    let country_index = country_element.selectedIndex;
    if (country_index >= 0) {
        print_state('state', country_index);
    }
});

// Fill countries dropdown on page load
